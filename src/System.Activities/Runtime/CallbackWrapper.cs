...
namespace System.Activities.Runtime
{
    ...
    internal class CallbackWrapper
    {
        ...
        //
        // When establising a callback, we need to Assert ReflectionPermission(MemberAccess) because
        // the callback method will typically be a private method within a class that derives from
        // Activity. Activity authors need to be aware that their callback may be invoked with
        // different permissions than were present when the Activity was originally executed and
        // perform appropriate security checks.
        //
        // We ensure that the declaring type of the callback method derives from Activity. This
        // check is made in RecreateCallback.
        //
        // The classes that derive from CallbackWrapper and call EnsureCallback do an explicit cast
        // of the returned delegate to the delegate type that they expect before calling thru to the
        // delegate. This cast is done in SecuritySafeCritical code.
        //
        ...
        internal void OnSerializing(StreamingContext context)
        {
            if (this.callbackName == null && !this.IsCallbackNull)
            {
                ...
                {
                    // If we're not directly off of the Activity type being used, then we need to
                    // store the declaringType's name.
        ...
        private Delegate RecreateCallback(Type delegateType, MethodInfo callbackMethod)
        {
            ...
            // If the declaring type does not derive from Activity, somebody has manipulated the
            // callback in the persistece store.
            ...
            // Asserting ReflectionPermission.MemberAccess because the callback method is most
            // likely internal or private
