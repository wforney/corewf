...
namespace System.Activities.Runtime
{
    ...
    internal partial class ActivityInstanceMap
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="ActivityInstanceMap" /> class.
        /// </summary>
        ...
        public void UpdateInstanceByActivityParticipation(ActivityExecutor activityExecutor, DynamicUpdateMap rootMap, ref Collection<ActivityBlockingUpdate>? updateErrors)
        {
            foreach (InstanceListNeedingUpdate participant in this.updateList)
            {
                if (participant.NothingChanged || participant.ParentIdShiftOnly)
                {
                    ...
                    // create a temporary NoChanges UpdateMap as well as a temporary no change
                    // MapEntry so that we can create a NativeActivityUpdateContext object in order
                    // to invoke UpdateInstance() on an activity which doesn't have a corresponding
                    // map and an map entry. The scenario enabled here is scheduling a newly added
                    // reference branch to a Parallel inside an activity's implementation.
            ...
            // Schedule evaluation of newly added arguments and newly added variables. This needs to
            // happen after all the invokations of UpdateInstance above, so that newly added
            // arguments and newly added variables get evaluated before any newly added activities
            // get executed. We iterate the list in reverse so that parents are always scheduled
            // after (and thus execute before) their children, which may depend on the parents.
            for (int i = this.updateList.Count - 1; i >= 0; i--)
            {
                ...
                {
                    // if the given InstanceList has no map entry, then there is no new
                    // LocationReferences to resolve.
                    continue;
                }
                ...
                {
                    // if there is no environment updates for this MapEntry, then there is no new
                    // LocationReferences to resolve.
                    continue;
                }
                ...
                for (int j = 0; j < participant.InstanceList.Count; j++)
                {
                    ...
                    {
                        // if the given ActivityInstance is not in Substate.Executing, then, do not
                        // try to resolve new LocationReferences
                        continue;
                    }
                    ...
                    if (envMap.HasPrivateVariableEntries && TryGatherSchedulableExpressions(envMap.PrivateVariableEntries, out addedPrivateVariableIndexes))
                    {
                        ...
                        // HasPrivateMemberChanged() check disallows addition of private variable
                        // default that offsets the private IdSpace, However, the added private
                        // variable default expression can be an imported activity, which has no
                        // affect on the private IdSpace. For such case, we want to be able to
                        // schedule the imported default expressions here.
        ...
        public void UpdateRawInstance(DynamicUpdateMap? updateMap, Activity targetDefinition, List<ActivityInstance> secondaryRootInstances, ref Collection<ActivityBlockingUpdate>? updateErrors)
        {
            ...
            {
                // error found. there is no need to proceed to updating the instances
                return;
            }

            // if UpdateType is either MapEntryExists or ParentIdShiftOnly, update the ActivityIDs
            // and update Environments also, update the ImplementationVersion.
            foreach (InstanceListNeedingUpdate update in this.updateList)
            {
                ...
                {
                    // this InstanceList must have been one of those whose IDs shifted by their
                    // parent's ID change, but no involvement in DU.
                    continue;
                }
        ...
        private static bool CanCompensationOrConfirmationHandlerReferenceAddedSymbols(InstanceList instanceList, DynamicUpdateMap rootUpdateMap, IdSpace rootIdSpace, List<ActivityInstance> secondaryRootInstances, ref Collection<ActivityBlockingUpdate> updateErrors)
        {
            for (int j = 0; j < instanceList.Count; j++)
            {
                ...
                // here, find out if the given non-default secondary root references an environment
                // to which a symbol is to be added via DU. we start from a secondary root instead
                // of starting from the enviroment with the already completed owner that was added
                // symbols. It is becuase for the case of adding symbols to noSymbols activities,
                // the environment doesn't even exist from which we can start looking for
                // referencing secondary root.
                ...
                {
                    // for each iteration of this for-loop, we are finding out if at every IdSpace
                    // level the map has any map entry whose activity has the CompensableActivity as
                    // an implementation decendant. The map may not exist for every IdSpace between
                    // the root and the CompensableActivity. If the matching map and the entry is
                    // found, then we find out if that matching entry's activity is a public
                    // decendant of any NoSymbols activity DU is to add variables or arguments to.
                    //
                    // This walk on the definition activity tree determines the hypothetical
                    // execution-time chain of instances and environments. The ultimate goal is to
                    // prevent adding variables or arguments to a NoSymbols activity which has
                    // already completed, but its decendant CompensableActivity's compensation or
                    // confirmation handlers in the future may need to reference the added variables
                    // or arguments.
                    ...
                    {
                        // the activity of this entry either has the CompensableActivity as an
                        // implementation decendant, or is the CompensableActivity itself.

                        // walk the same-IdSpace-parent chain of the entry, look for an entry whose
                        // EnvironmentUpdateMap.IsAdditionToNoSymbols is true.
        ...
        // searching secondaryRootInstances list is necessary because instance in InstanceList
        // doesn't have its Parent set until it's fixed up. so the only way to find out if an
        // instance in InstanceList is a secondary root is to lookup in secondaryRootInstances list.
        private static bool IsNonDefaultSecondaryRoot(ActivityInstance instance, List<ActivityInstance> secondaryRootInstances)
        {
            ...
            {
                // Non-default secondary roots are CompensationParticipant type, and their
                // environment will always have a non-null parent which is the environment owned by
                // a CompensableActivity. A secondary root whose environment parent is null is the
                // default secondary root, WorkflowCompensationBehavior.
        ...
        private IList<InstanceListNeedingUpdate> GetInstanceListsNeedingUpdate(DynamicUpdateMap? updateMap, Activity targetDefinition, List<ActivityInstance> secondaryRootInstances, ref Collection<ActivityBlockingUpdate>? updateErrors)
        {
            ...
            for (int i = 0; i < this.rawDeserializedLists.Length; i++)
            {
                ...
                else if (updatedActivity.MapEntry == null)
                {
                    ...
                    {
                        // this newQualifiedId is the new id for those InstanceLists whose IDs
                        // shifted by their parents' ID change
                ...
                {
                    // no validation error for this InstanceList add it to the list of InstanceLists
                    // to be updated
                    update = new InstanceListNeedingUpdate
                    {
                        InstanceList = list,
        ...
        private class InstanceListNeedingUpdate
        {
            ...
            // A pointer to this activity, in the new definition. Null if we don't have the
            // definition loaded.
            ...
            // the following three properties are mutual exlusive, meaning, one and only one of them
            // evaluates to TRUE.
            ...
            // The Map & MapEntry for this ActivityId, if there is one. Null if the activity's
            // parent Id was updated, but not the activity itself, Or null if nothing changed.
