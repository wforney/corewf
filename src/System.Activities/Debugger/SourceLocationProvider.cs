...
namespace System.Activities.Debugger
{
    ...
    // point with Workflow project system (TBD). The current plan is to get SourceLocation from (in
    // this order):
    // 1. pdb (when available) 2a. parse xaml files available in the same project (or metadata
    // store) or 2b. ask user to point to the correct xaml source.
    ...
    public static class SourceLocationProvider
    {
        ...
        public static ICollection<ActivitySymbol> GetSymbols(Activity rootActivity, Dictionary<object, SourceLocation> sourceLocations)
        {
            ...
            var realRoot = (rootActivity.RootActivity != null) ? rootActivity.RootActivity : rootActivity;
            ...
            while (activitiesRemaining.Count > 0)
            {
                ...
                var origin = currentActivity.Origin == null ? currentActivity : currentActivity.Origin;
        ...
        static internal Dictionary<object, SourceLocation> GetSourceLocations(Activity rootActivity, out string sourcePath, out bool isTemporaryFile, out byte[] checksum)
        {
            ...
            if (!string.IsNullOrEmpty(symbolString))
            {
                try
                {
                    ...
                    if (wfSymbol != null)
                    {
                        ...
                        // rootActivity.RootActivity is the workflow root activity. if they are not
                        // the same, then it must be compiled XAML, because loose XAML (i.e. XAMLX)
                        // always have the symbol attached at the root.
            ...
            var permissionRevertNeeded = false;
            ...
            try
            {
                ...
                else
                {
                    ...
                    // Need to store the file in memory temporary so don't have to re-read the file
                    // twice for XamlDebugXmlReader's BracketLocator. If there is a debugger
                    // attached, Assert FileIOPermission for Read access to the specific file.
                    if (System.Diagnostics.Debugger.IsAttached)
                    {
                        permissionRevertNeeded = true;
                        var permission = new FileIOPermission(FileIOPermissionAccess.Read, sourcePath);
                        permission.Assert();
                    }
                    ...
                    var debuggableWorkflowTree = deserializedObject as IDebuggableWorkflowTree;
        ...
        // For most of the time, we need source location for object that appear on XAML. During
        // debugging, however, we must not transform the internal activity to their origin to make
        // sure it stop when the internal activity is about the execute Therefore, in debugger
        // scenario, translateInternalActivityToOrigin will be set to false.
        internal static Dictionary<object, SourceLocation> GetSourceLocations(Activity rootActivity, WorkflowSymbol symbol, bool translateInternalActivityToOrigin)
        {
            ...
            // Make sure the qid we are using to TryGetElementFromRoot are shifted appropriately
            // such that the first digit that QID is the same as the last digit of the rootActivity.QualifiedId.

            var rootIdArray = rootActivity.QualifiedId.AsIDArray();
            var idOffset = rootIdArray[rootIdArray.Length - 1] - 1;
