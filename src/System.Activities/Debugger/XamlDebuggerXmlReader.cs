...
namespace System.Activities.Debugger
{
    ...
    public class XamlDebuggerXmlReader : XamlReader, IXamlLineInfo
    {
        ...
        // A XamlReader that need to collect source level information is necessary the one that is
        // closest to the source document. This constructor is fundamentally flawed because it
        // allows any XAML reader Which could output some XAML node that does not correspond to source.
        ...
        // This one is worse because in implementation we expect the same object instance through
        // two parameters.
        [Obsolete("Don't use this constructor. Use \"public XamlDebuggerXmlReader(TextReader underlyingTextReader)\" or \"public XamlDebuggerXmlReader(TextReader underlyingTextReader, XamlSchemaContext schemaContext)\" instead.")]
                ...
                if (readSucceed)
                {
                    ...
                    switch (this.Current.NodeType)
                    {
                        case XamlNodeType.StartMember:
                            ...
                            // When we reach a StartMember node, the next node to come might be a
                            // Value. To correctly pass SourceLocation information, we need to
                            // rewrite this node to use ValueNodeXamlMemberInvoker. But we don't
                            // know if the next node is a Value node yet, so we are buffering here
                            // and look ahead for a single node.
                            ...
                            // object element supplies the type construction information for the
                            // surrounding object element.
                            var isInitializationValue = this.Current.Member == XamlLanguage.Initialization;

                            var moreNode = this.underlyingReader.Read();
                            ...
                            // It is possible that the next node after StartMember is a
                            // StartObject/GetObject. We need to push the object declaration node to
                            // the Stack
                            this.PushObjectDeclarationNodeIfApplicable();
                                ...
                                && this.Current.NodeType == XamlNodeType.Value)
                            {
                                ...
                                var currentLocation = new DocumentLocation(this.Current.LineNumber, this.Current.LinePosition);
                                var isInAttribute = this.xmlReaderWithSourceLocation.AttributeValueRanges.TryGetValue(currentLocation, out valueRange);
                                var isInContent = isInAttribute ? false : this.xmlReaderWithSourceLocation.ContentValueRanges.TryGetValue(currentLocation, out valueRange);

                                if (isInAttribute || (isInContent && !isInitializationValue))
                                {
                                    // For Value Node with known line info, we want to route the
                                    // value setting process through this Reader. Therefore we need
                                    // to go back to the member node and replace the XamlMemberInvoker.
                                    var startMemberNodeForValue = this.bufferedXamlNodes.Peek();
                                ...
                                {
                                    var currentStartObject = this.objectDeclarationRecords.Peek();
        ...
        private XamlMember CreateAttachableMember(MethodInfo getter, MethodInfo setter, SourceLocationMemberType memberType)
        {
            ...
            var invoker = new SourceLocationMemberInvoker(this.SourceLocationCollector, memberType);
        ...
        {
            var startNode = this.objectDeclarationRecords.Pop();
                ...
                && (startNode.Type != null && !startNode.Type.IsUnknown && !startNode.Type.IsMarkupExtension))
            {
                ...
                {
                    var myEndLocation = new DocumentLocation(this.Current.LineNumber, this.Current.LinePosition);
                ...
                var myRealEndBracket = new DocumentLocation(myEndBracket.LineNumber.Value, myEndBracket.LinePosition.Value + 1);
            ...
            // We need to make sure we also buffer the current node so that this is not missed when
            // the buffer exhausts.
            this.bufferedXamlNodes.Enqueue(this.Current);
        }
        ...
        {
            var argumentInstance = instance as Argument;

            // For Argument containing an IValueSerializable expression serializing as a ValueNode.
            // We associate the SourceLocation to the expression instead of the Argument. For
            // example, when we have
            // <WriteLine Text="[abc]" />
            // , Then the SourceLocation found for the InArgument object is associated with the
            // VisualBasicValue object instead.
        ...
        private void OnValueNodeDeserialized(object value, DocumentRange attributeValueLocation)
        {
            ...
            // XamlDebuggerXmlReader has no idea what the filename is (it only knew a stream of
            // data) So we set FileName = null.
        ...
        private class XamlSourceLocationCollector
        {
            ...
            // in the case: <InArgument x:TypeArguments="x:String">["abc" + ""]</InArgument>
            // instance is a Argument, with a VB Expression. We hope, the VB expression got
            // notified, too.
