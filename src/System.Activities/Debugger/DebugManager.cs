...
namespace System.Activities.Debugger
{
    ...
    {
        private static StateManager.DynamicModuleManager dynamicModuleManager;
        ...
        private Dictionary<object, State> states;
        ...
        // Whether debugging is done from the start of the root workflow, contrast to attaching into
        // the middle of a running workflow.
        ...
        // Exiting the DebugManager. Delete all temporary files
        public void Exit()
        {
        ...
        // Main instrumentation. Currently the typeNamePrefix is used to notify the Designer of
        // which file to show. This will no longer necessary when the callstack API can give us the
        ...
        internal void Instrument(Activity activity)
        {
            ...
            foreach (Activity sameSourceActivity in sameSourceActivities)
            {
                ...
                // Mark it as instrumentated, even though it fails so it won't be retried.
        ...
        private static Dictionary<string, object> GenerateLocals(ActivityInstance instance)
        {
        ...
        // Create logical thread and bring its call stack to reflect call from the root up to (but
        // not including) the instance. If the activity is an expression though, then the call stack
        // will also include the instance (since it is the parent of the expression).
        private int CreateLogicalThread(Activity activity, ActivityInstance instance, bool primeCurrentInstance)
        {
        ...
        // Test whether activity has been instrumented. If not, try to instrument it. It will return
        // true if instrumentation is already done or instrumentation is succesful. False otherwise.
        ...
        // Given an activity, return the thread id where it is currently executed (on the top of the
        // callstack). Boolean "strict" parameter determine whether the activity itself should be on
        // top of the stack. Strict checking is needed in the case of "Leave"-ing a state.
        // Non-strict checking is needed for "Enter"-ing a state, since the direct parent of the
        // activity may not yet be executed (e.g. the activity is an argument of another activity,
        ...
        // Get threads currently executing the parent of the given activity, if none then create a
        // new one and prep the call stack to current state.
        private int GetOrCreateThreadId(Activity activity, ActivityInstance instance)
        {
            int threadId = -1;
        ...
        // Workflow rooted at rootActivity1 and rootActivity2 have same source file, but they are
        // two different instantiation. rootActivity1 has been instrumented and its instrumentation
        // states can be re-used by rootActivity2.
        //
        // MapInstrumentationStates will walk both Workflow trees in parallel and map every state
        // for activities in rootActivity1 to corresponding activities in rootActivity2.
            ...
            while (pairsRemaining.Count > 0)
            {
                ...
                while (hasNextItem1 && hasNextItem2)
                {
                    if (!visited.Contains(enumerator1.Current))  // avoid adding the same activity (e.g. some default implementation).
                    {
                        ...
                        {
                            // Give debugger log instead of just asserting; to help user find out
                            // mismatch problem.
                            Trace.WriteLine(
                ...
                // If enumerators do not finish at the same time, then they have unmatched number of
                // activities. Give debugger log instead of just asserting; to help user find out
                // mismatch problem.
        ...
        private void PrimeCallStack(int threadId, Stack<ActivityInstance> ancestors)
        {
