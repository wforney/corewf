...
namespace System.Activities.Statements
{
    ...
    public sealed class CompensableActivity : NativeActivity<CompensationToken>
    {
                ...
                Body = new ActivityAction<CompensableActivity, ValidationContext>
                {
                    ...
                    Handler = new Sequence
                    {
                        ...
                        Activities =
                        {
                            ...
                            // Need to replace the lambda expression with a CodeActivity for partial
                            // trust. new While(env => (assertFlag.Get(env) != false) &&
                            ...
                            new While
                            {
                                ...
                                Body = new Sequence
                                {
                                    Activities =
                                    {
                                        // Need to replace the lambda expression with a CodeActivity
                                        // for partial trust. new If(env =>
                                        // (elements.Get(env).ElementAt(index.Get(env))).GetType()
                                        ...
                                        {
                                            To = index,
                                            // Need to replace the lambda expression for partial
                                            // trust. Using Add expression activity instead of a
                                            // CodeActivity here. Value = new InArgument<int>(env =>
                                            // index.Get(env) + 1)
        ...
        private void OnBodyExecutionComplete(NativeActivityContext context, ActivityInstance completedInstance)
        {
            ...
            {
                // we check for faulted as well for one odd case where an exception can be thrown
                // from the body activity itself.
    ...
    // variables. So this code activity replaces the lambda expression in this statement: While(env
    // => (assertFlag.Get(env) != false) && index.Get(env) < elements.Get(env).Count())
