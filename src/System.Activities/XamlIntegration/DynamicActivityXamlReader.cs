...
namespace System.Activities.XamlIntegration
{
    ...
    // type "Activity" to member "DynamicActivity" Rewrites x:Class to DynamicActivity.Name
    ...
    internal class DynamicActivityXamlReader : XamlReader, IXamlLineInfo
    {
        ...
        // Properties are tricky since they support default values, and those values can appear
        // anywhere in the XAML document. So we need to buffer their XAML nodes and present them
        // only at the end of the document (right before the document end tag), when we have both
        // the declaration and the value realized.
        private BufferedPropertyList bufferedProperties;

        // in the ActivityBuilder case we need to jump through some extra hoops to support
        // PropertyReferenceExtension, since in the ActivityBuilder case Implementation isn't a
        // template (Func<Activity>), so we need to map such members into attached properties on
        // their parent object
            ...
            : base()
        {
            ...
            if (innerReader is IXamlLineInfo lineInfo && lineInfo.HasLineInfo)
            {
        ...
        public override bool Read()
        {
            ...
            {
                // now that we've mapped the innerReader into (at least) one node entry, pump that
                // reader as well
                return this.nodeReader.Read();
            }
        }
        ...
        private void ExitObject()
        {
            ...
            {
                // we're about to write out the last tag. Dump our accrued properties as no more
                // property values are forthcoming.
        ...
        private bool IsXClassName(XamlType xamlType)
        {
            ...
            // GetXamlNamespaces is a superset of PreferredXamlNamespace, it's not a must for the
            // above code to check for preferredXamlNamespace, but since the old code uses
            // .Contains(), which was a minor bug, we decide to use StartsWith in new code and keep
            // the old code for back compatible reason.
        ...
        // pull on our inner reader, map the results as necessary, and pump mapped results into the
        // streaming node reader that we're offering up. return true if we need to keep pumping
        // (because we've buffered some nodes on the side)
        private bool ProcessCurrentNode()
        {
            ...
            switch (this.innerReader.NodeType)
            {
                case XamlNodeType.StartMember:
                    ...
                    // find out if the member is a default value for one of our declared properties.
                    // If it is, then we have a complex case where we need to:
                    // 1) read the nodes into a side list
                    // 2) interleave these nodes with the DynamicActivityProperty nodes since they
                    // need to appear as DynamicActivityProperty.Value
                    ...
                    else
                    {
                        ...
                        if (this.depth == 2)
                        {
                            ...
                            else if (currentMember == XamlLanguage.ClassAttributes)
                            {
                                ...
                                // x:ClassAttributes directive has no following GetObject, but
                                // Attributes does since it's not a directive
        ...
        // used for a number of cases when wrapping we need to add a GetObject/StartMember(_Items)
        // since XAML directives intrinsically take care of it
        private void WriteWrappedMember(bool stripWhitespace)
        {
            ...
            // close the GetObject added above. Note that we are doing EndObject/EndMember after the
            // last node (EndMember) rather than inserting EndMember/EndObject before the last
            // EndMember since all EndMembers are interchangable from a state perspective
        ...
        // <DynamicActivityProperty> nodes) with their corresponding default values <MyClass.Foo>
        // (to be rewritten as <DynamicActivityProperty.Value> nodes). Definitions come all at once,
        // but values could come anywhere in the XAML document, so we save them all almost until the
        // end of the document and write them all out at once using BufferedPropertyList.CopyTo().
        private class BufferedPropertyList
        {
            private readonly XamlNodeQueue outerNodes;
            private readonly DynamicActivityXamlReader parent;
            ...
            public void BufferDefinitions(DynamicActivityXamlReader parent)
            {
                ...
                // x:Members directive has no following GetObject, but Properties does since it's
                // not a directive
                ...
                // 2) process the subnodes and store them in either ActivityPropertyHolders, or
                // exigent nodes in the outer node list
                ...
                while (continueReading)
                {
                        ...
                        && subReader.Type == XamlLanguage.Property)
                    {
                        ...
                        // ActivityPropertyHolder consumed the subtree, so we don't need to pump a
                        // Read() in this path
                    }
                    else
                    {
                ...
                // close the GetObject added above. Note that we are doing EndObject/EndMember after
                // the last node (EndMember) rather than inserting EndMember/EndObject before the
                // last EndMember since all EndMembers are interchangable from a state perspective
            ...
            public void FlushTo(XamlNodeQueue targetNodeQueue, DynamicActivityXamlReader parent)
            {
                ...
                while (sourceReader.Read())
                {
                    ...
                    {
                        if (sourceReader.Value is ActivityPropertyHolder propertyHolder)
                        {
            ...
            {
                // We've seen all the property definitions we're going to see. Write out any values
                // already accumulated.

                // If we have picked up any values already before definitions, process them
                // immediately (and throw as usual if corresponding definition doesn't exist)
                if (this.valueHolders != null)
                {
            ...
            private class ActivityPropertyHolder
            {
                ...
                private readonly XamlNodeQueue nodes;
                ...
                public ActivityPropertyHolder(DynamicActivityXamlReader parent, XamlReader reader)
                {
                    ...
                    // parse the subtree, and extract out the Name and Type for now. keep the
                    // node-list open for now, just in case a default value appears later in the document
                    ...
                    while (continueReading)
                    {
                        ...
                        {
                            case XamlNodeType.StartMember:
                                // map <x:Property> membes to the appropriate
                                // <DynamicActivity.Property> members
                ...
                // called when we've reached the end of the activity and need to extract out the
                // resulting data into our activity-wide node list
                ...
                public void ProcessDefaultValue(XamlMember propertyValue, XamlReader subReader, IXamlLineInfo lineInfo)
                {
                    ...
                    // temporary hack: read past GetObject/StartMember nodes that are added by the
                    // XAML stack. This has been fixed in the WPF branch, but we haven't FI'ed that yet
                    ...
                    // Add SO tag if necessary UNLESS there's no value to wrap (which means we're
                    // already at EO)
        ...
        {
            private readonly XamlMember activityBuilderPropertyReferencesMember;
            private readonly XamlMember activityPropertyReferenceSourceProperty;
            private readonly XamlMember activityPropertyReferenceTargetProperty;
            private readonly XamlType activityPropertyReferenceXamlType;
            private readonly DynamicActivityXamlReader parent;
            private readonly Stack<Frame> stack;
            private MemberInformation bufferedMember;
            ...
            // Whenever we encounter a StartMember, we buffer it (and any namespace nodes folllowing
            // it) until we see its contents (SO/GO/V). If the content is a
            // PropertyReferenceExtension, then we convert it to an ActivityPropertyReference in the
            // parent object's ActivityBuilder.PropertyReference collection, and dont' write out the
            // member. If the content is not a PropertyReferenceExtension, or there's no content
            // (i.e. we hit an EM), we flush the buffered SM + NS*, and continue as normal.
            ...
            private bool IsExpectedPropertyReferenceMember(XamlReader reader)
            {
            ...
            private bool IsPropertyReferenceExtension(XamlType type)
            {
            ...
            private string ReadPropertyName(XamlReader reader)
            {
                ...
                {
                    // Our parent will never see this EndMember node so we need to force its depth
                    // count to decrement
            ...
            private string ReadPropertyReferenceExtensionPropertyName(XamlReader reader)
            {
                ...
                while (!reader.IsEof && reader.NodeType != XamlNodeType.EndObject)
                {
                    ...
                    {
                        // unexpected members. For compat with 4.0, unexpected members on
                        // PropertyReferenceExtension are silently ignored
                        reader.Skip();
                    }
                }
            ...
            private void WritePropertyReferenceCollection(XamlNodeQueue serializedReferences, XamlWriter targetWriter, IXamlLineInfo lineInfo)
            {
            ...
            private void WritePropertyReferenceFrameToParent(MemberInformation targetMember, string sourceProperty, Frame parentFrame, IXamlLineInfo lineInfo)
            {
                ...
                // we didn't write out the target StartMember, so suppress the EndMember
                parentFrame.SuppressNextEndMember = true;
            }

            private void WriteSerializedPropertyReference(XamlWriter targetWriter, IXamlLineInfo lineInfo, string targetName, string sourceName)
            {
                // Line Info for the entire <ActivityPropertyReference> element comes from the end
                // of the <PropertyReference> tag
